...
def generate_model(n_timesteps, *, n_channels=1, n_components=2, kernel_size=3,
    ...
    pixel_cnn.add(keras.layers.Dense(n_components*3))
    
    
    pixel_cnn.add(keras.layers.Lambda(lambda x: tf.add(tf.math.softplus(x),1e-3)))
    '''
    ...
    '''
    #pixel_cnn.add(keras.layers.Lambda(lambda x: tf.reshape(x,(-1,n_timesteps,n_components,3))))
    pixel_cnn.add(
        tfp.layers.DistributionLambda(
            make_distribution_fn=lambda t: tfd.MixtureSameFamily(
                mixture_distribution=tfd.Categorical(probs=t[...,0:n_components]),
                components_distribution=tfd.Beta(
                    concentration1=t[...,n_components:2*n_components],
                    concentration0=t[...,2*n_components:]
                )
            )
        )
    )
