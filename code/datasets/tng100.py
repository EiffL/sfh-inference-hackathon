...
def find_summaries(mass, time, percentiles=np.linspace(0.1, 0.9, 9)):
    ...
    for percentile in percentiles:
        summary_mass = min(
            mass, key=lambda x: abs(x - mass[0] * percentile)
        )  # find mass closest to the half mass
        summary_masses.append(summary_mass)
        summary_mass_indices = np.where(mass == summary_mass)[
            0
        ]  # find the corresponding indices
        summary_mass_index = summary_mass_indices[
            0
        ]  # chose the first index for the half mass
            ...
            features=tfds.features.FeaturesDict(
                {
                    ...
                    "mass_quantiles": tfds.features.Tensor(
                        shape=(9,), dtype=tf.float32
                    ),
    ...
    def _generate_examples(self, root_path):
        ...
        for filename in glob.glob(root_path + "/cats_SFH/*.csv"):
            ...
            try:
                ...
                img = stack_bands(
                    root_path + "/images/TNG100/sdss/sn99/noiseless/", object_id
                )
                ...
                example.update(
                    {"sed": np.array(list(row.values())[1:]).astype("float32")}
                )
                ...
                example.update(
                    {
                        k: interp1d(
                            sfh["time"], sfh[k], bounds_error=False, fill_value=0.0
                        )(a).astype("float32")
                        for k in [
                            "SFR_halfRad",
                            "SFR_Rad",
                ...
                mass_history_summaries = find_summaries(
                    example["Mstar_Half"], example["time"]
                )
                last_over_max = example["Mstar_Half"][0] / np.max(example["Mstar_Half"])
                example.update(
                    {
                        "mass_quantiles": mass_history_summaries,
